...
def closest_obs_station(lat: float, lon: float, web_lookup: bool = False):
    ...
    def geo_distance(lat1: float, lon1: float, lat2: float,
                     lon2: float) -> float:
    ...
    closest_wmo = min(
        stations,
        key=
        lambda wmo: geo_distance(lat, lon, stations[wmo]['lat'], stations[wmo]['lon'])
    )
...
def get_product_list():
    ...
    """
    products = dict()
    pattern = (r'<a href="/products/(?P<product>ID[A-Z]\d\d\d\d\d)/'
